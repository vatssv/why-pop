...
def load_image_from_file(filename, shape):
  ...
  try:
    ...
    # print('Loaded single image: ', img, img.shape)
...
def get_acts_from_images(imgs, model, bottleneck_name):
  ...
  """
  activation_layer = model.create_intermediate_model(bottleneck_name)
  return np.asarray(model.run_examples(imgs, activation_layer, bottleneck_name)).squeeze()
...
def plot_concepts(cd, bn, num=10, address=None, mode='diverse', concepts=None):
  ...
  for n, concept in enumerate(concepts):
    ...
    concept_image_names = cd.dic[bn][concept]['image_names']
    if mode == 'max':
    ...
    for i, idx in enumerate(idxs):
      fig_individual = plt.figure(figsize=(4, 2))
      ax_ind = plt.Subplot(fig_individual, 121)
      ax_ind.imshow(concept_images[idx])
      ax_ind.set_xticks([])
      ax_ind.set_yticks([])
      ax_ind.grid(False)
      fig_individual.add_subplot(ax_ind)
      ...
      original_image_name = concept_image_names[idx]
      marked_image = mark_boundaries(image, mask, color=(1, 1, 0), mode='thick')
      ...
      ax_ind = plt.Subplot(fig_individual, 122)
      ax_ind.imshow(marked_image)
      ax_ind.set_xticks([])
      ax_ind.set_yticks([])
      ax_ind.grid(False)
      fig_individual.add_subplot(ax_ind)
      track_id = original_image_name.split('/')[-1].split('_')[0]
      with tf.io.gfile.GFile(address + bn + '.' + concept + '.' + str(i) + '_' + str(track_id) + '.png', 'w') as f:
        fig_individual.savefig(f)
...
def save_concepts(cd, concepts_dir):
  ...
  for bn in cd.bottlenecks:
    for concept in cd.dic[bn]['concepts']:
      ...
      image_names = cd.dic[bn][concept]['image_names']
      ...
      for i in range(len(images)):
        og_name = image_names[i].split('/')[-1].split('_')[0]
        image_name = '0' * int(np.ceil(2 - np.log10(i + 1))) + '{}_{}_{}'.format(
            i + 1, image_numbers[i], og_name)
