...
def main(args):
  ...
  cd.create_patches(method='slic', param_dict={'n_segments': [10, 15, 25]})
  ...
  addresses = []
  base_path = image_dir
  for i, n in enumerate(cd.discovery_image_names):
    track_id = n.split('/')[-1].split('_')[0]
    image_name = os.path.join(base_path, '0' * (3 - int(np.log10(i + 1))) + str(i + 1) + '_' + track_id + '.png')
    addresses.append(image_name)
  # print('Save images lengths: ', len(cd.discovery_images), len(cd.discovery_image_names))
  # print('Last element of dataset: ', cd.dataset[-1])
#   print('Dict: ', cd.dic[args.bottleneck.split(',')[0]])
  ace_helpers.save_images(addresses,
                            (cd.discovery_images * 256).astype(np.uint8))
  # Discovering Concepts
  cd.discover_concepts(method='KM', param_dicts={'n_clusters': 25})
  dict_dump_dir = os.path.join(discovered_concepts_dir, f'{args.target_class}_concept_dic.txt')
  with open(dict_dump_dir, 'w') as file:
    for key, value in cd.dic.items():
  ...
  # print('cd before saving concepts: ', cd.dic)
