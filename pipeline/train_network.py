...
class Classifier(object):
    ...
    def my_dataloader_collate(self, batch):
        ...
        if len_batch > len(batch): # if there are samples missing just use existing members, doesn't work if you reject every sample in a batch
            ...
            for i in range(diff):
                batch.append(self.dataset[np.random.randint(0, len(self.train_dl))])
    ...
    def generate_batch(self):
        ...
        self.train_dl = torch.utils.data.DataLoader(self.train_data, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_dataloader_collate)
        self.val_dl = torch.utils.data.DataLoader(self.validation_data, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_dataloader_collate)
        self.test_dl = torch.utils.data.DataLoader(self.test_data, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_dataloader_collate)
...
def main():
    ...
    clf.train_torch_model('/mnt/f/Code/Thesis/model_torch_3.pth', writer, timestamp, retrain=True)
