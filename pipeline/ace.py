...
class ConceptDiscovery(object):
    ...
    if self.num_workers:
      ...
      for fn, sp_outputs in enumerate(outputs):
        ...
        for superpixel, patch in zip(image_superpixels, image_patches):
          ...
          image_names.append(filenames[fn])
    else:
      for fn, img in enumerate(self.discovery_images):
        print('Loaded raw image shape: ', img.shape)
        ...
        for superpixel, patch in zip(image_superpixels, image_patches):
          ...
          image_numbers.append(fn)
          image_names.append(filenames[fn])
    self.dataset, self.image_numbers, self.patches, self.image_names =\
    np.array(dataset), np.array(image_numbers), np.array(patches), np.array(image_names)
  ...
  def _return_superpixels(self, img, method='slic',
    ...
    """
    print('Calculating super pixels.')
    if param_dict is None:
      param_dict = {}
    if method == 'slic':
    ...
    for i in range(n_params):
      ...
      if method == 'slic':
        segments = segmentation.slic(
            ...
            sigma=sigmas[i])
        # print('Segment shape: ', segments.shape, segments)
      elif method == 'watershed':
  ...
  def _extract_patch(self, image, mask):
    ...
    # print('Mask and image shape: ', mask.shape, image.shape)
  ...
  def _patch_activations(self, imgs, bottleneck, bs=100, channel_mean=None):
    ...
    """
    print('Finding patch activations. Total images: ', len(self.dataset))

    activation_model = self.model.create_intermediate_model(bottleneck)
    ...
    if self.num_workers:
      pool = multiprocessing.Pool(self.num_workers)
      output = pool.map(
          lambda i: self.model.run_examples(imgs[i * bs:(i + 1) * bs], activation_model, bottleneck),
          np.arange(int(imgs.shape[0] / bs) + 1))
    else:
      output = []
      for i in range(int(imgs.shape[0] / bs) + 1):
        # print('Type and length of images passed: ', type(imgs[i * bs:(i + 1) * bs]), len(imgs[i * bs:(i + 1) * bs]))
        output.append(
            self.model.run_examples(imgs[i * bs:(i + 1) * bs], activation_model, bottleneck))
    # print('Final output from activations was ', output, output.shape)
    # print('Last couple of images: ', imgs[-2:])
        ...
        if len(label_idxs) > self.min_imgs:
          ...
          if highly_common_concept or cond2 or cond3:
            ...
            bn_dic[concept] = {
                ...
                'image_names': self.image_names[concept_idxs]
            }
  ...
  def _random_concept_activations(self, bottleneck, random_concept):
    ...
    if not tf.io.gfile.exists(rnd_acts_path):
      rnd_imgs, rnd_img_filenames = self.load_concept_imgs(random_concept, self.max_imgs)
  ...
  def _return_gradients(self, images):
    ...
    gradients = {}
    print('Class label for this set is: ', self.target_class, self.target_class.replace('_', ' '))
    class_id = self.model.label_to_id(self.target_class.replace('_', ' '))
    print('Class id for this set is: ', class_id)
    logit_model = self.model.create_intermediate_model('fc')
    for bn in self.bottlenecks:
      acts = get_acts_from_images(images, self.model, bn)
      # print('Activations and its shape is: ', acts, acts.shape)
      bn_grads = np.zeros((acts.shape[0], np.prod(acts.shape[1:])))
      print('bn_grads shape: ', bn_grads.shape)
      result_gradients = self.model.get_gradient(acts, [class_id], logit_model, images, bn)
      for i in range(len(acts)):
        bn_grads[i] = result_gradients[i].reshape(-1)
  ...
  def tcavs(self, test=False, sort=True, tcav_score_images=None):
    ...
    if tcav_score_images is None:  # Load target class images if not given
      raw_imgs, raw_imgs_filenames = self.load_concept_imgs(self.target_class, 2 * self.max_imgs)
      tcav_score_images = raw_imgs[-self.max_imgs:]
    gradients = self._return_gradients(tcav_score_images)
    print('Loaded images: ', len(raw_imgs))
    for bn in self.bottlenecks:
